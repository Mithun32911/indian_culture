...
const TourGuideLogin = () => {
  ...
  useEffect(() => {
    // If user was redirected from protected content, show a message
    const urlParams = new URLSearchParams(location.search);
    if (urlParams.get('redirect') === 'protected-content') {
      setRedirectMessage(t('tourGuideLogin.redirect_prompt'));
    }
  }, [location]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('TourGuideLogin error:', err);
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginTop: '1rem' }}>
            ...
            <div style={{ marginTop: 8 }}>
              <a href="#" onClick={() => navigate('/admin/tour-guide-signup')}>{t('userLogin.no_account')}</a>
              {' | '}
